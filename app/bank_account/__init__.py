...
from app.bank_account.forms import csv_upload
from werkzeug.utils import secure_filename, redirect

bank_account = Blueprint('bank_account', __name__,
...
def browse_account_transactions(page):
    page = page
    per_page = 1000
    # display account transactions for all users
...
def transaction_upload():
    ...
    if form.validate_on_submit():
        ...
        # log the transaction file
        log.info('Bank acccount transaction file uploaded filepath: '+ filepath)

        with open(filepath, encoding='utf-8-sig') as file:
            csv_file = csv.DictReader(file)
            for row in csv_file:
                amount = int(row['AMOUNT'])
                # as amount already in plus & minus so directly adding to current balance.
                # otherwise we could have been add if type is CREDIT and subtract if DEBIT.
